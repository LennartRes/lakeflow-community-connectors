...
# Check if places_flattened table exists and has data
...
if distance_matrix_table_ready:
...
# Check if places_intelligence table exists (depends on places data)
...
# Check if geocoder silver tables exist
geocoder_flat_ready = table_has_data(SILVER_CATALOG, SILVER_SCHEMA, "geocoder_flattened")
...
if geocoder_flat_ready:
    ...
    if geocoder_parsed_ready:
        ...
        # Simpler version without parsed address components
        geocoder_enriched = geocoder_flat.withColumn(
            # Distance from reference
            "distance_from_reference_km",
            spark_round(
                haversine_distance(
            ...
            col("place_id"),
            col("formatted_address"),
            lit(None).alias("street_address"),
            lit(None).alias("city"),
            lit(None).alias("state_province"),
            lit(None).alias("country"),
            lit(None).alias("postal_code"),
            col("latitude"),
            col("longitude"),
            col("location_type"),
            col("plus_code_global"),
...
try:
    ...
    if gold_tables:
        ...
        print("No tables created in gold layer.")
except Exception as e:
